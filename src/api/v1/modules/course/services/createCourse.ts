...
// Can't we use this fucntion?

const AIGenContent = async (title: string) => {
  ...
  return { courseDescription, courseCategory, courseImage }
}
...
const parseFiles = async (files: File[]) => {
  ...
  const fileUrls = fileData.map((file: any) => file.fileUrl)
  const fileContent = fileData.map((file: any) => file.data)

  return { fileUrls, fileContent, error }
}
...
const createCourse = async ({
  ...
  // const {courseDescription, courseCategory, courseImage} = AIGenContent(title)
  const validCourseTitle = await AIGen.confirmCourseTitle(title)
  if (!validCourseTitle)
    return {
      courseId: newCourse,
  ...
  if (files) {
    // const {fileUrls, fileContent, error} = parseFiles(files)
    console.log('Starting work on files')
    // Add files to course and generate course topics
